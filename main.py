...
def create_color_map(unique_colors):
    ...
    for color in sorted(list(unique_colors)):  # Sort for deterministic assignment
        if not char_pool:
            ...
            next_char = "?"
            if (
                color not in color_to_char
            ):  # Avoid overwriting if '?' was already assigned
    ...
    if " " in char_to_color:
        print(
            "Warning: Space character (' ') was assigned to a color. This might conflict with transparency."
        )
...
def generate_palette_file(char_to_color_map, output_path):
    ...
    try:
        with open(output_path, "w") as f:
            f.write(
                "// Add these lines to your Java project where you initialize GameView\n"
            )
            ...
            for char, color in sorted(char_to_color_map.items()):
                ...
                if not is_default:
                    ...
                    java_char = (
                        f"'{char}'" if char != "'" else "'\\''"
                    )  # Handle single quote itself
                    java_char = (
                        java_char if char != "\\" else "'\\\\'"
                    )  # Handle backslash itself
                ...
                for x in range(width):
                    ...
                    if a == 0:  # Fully transparent is space
                        ...
                        if char:
                            ...
                            print(
                                f"Warning: Color ({r},{g},{b}) with alpha {a} not found in map for {image_path}. Using '?'."
                            )
                            line += "?"
    ...
    try:
        with open(output_path, "w") as f:
            ...
            for image_path in image_files:
                ...
                variable_name = "".join(
                    c if c.isalnum() else "_" for c in name_without_ext
                ).upper()
                ...
                f.write("     */\n")
                f.write(
                    f"    public static final String {variable_name} = {block_string};\n\n"
...
if __name__ == "__main__":
    ...
    if not image_files:
        ...
        print(
            "   the correct package folder within your Java project's source directory"
        )
        ...
        print(
            "4. In your Java code, access the block images using the static constants,"
        )
        print(
            "   for example: `gameView.addBlockImageToCanvas(GameBlockImages.TREE, x, y, size, rotation);`"
        )
        print("----------------------------------------------------------")
